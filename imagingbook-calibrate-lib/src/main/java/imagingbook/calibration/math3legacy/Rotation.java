...
public class Rotation implements Serializable {
    ...
    static final String ZERO_NORM_FOR_ROTATION_AXIS = "zero norm for rotation axis";
    static final String ROTATION_MATRIX_DIMENSIONS = "a {0}x{1} matrix cannot be a rotation matrix";
   ...
   // ------------------------------------------------------------
    ...
    public Rotation(double q0, double q1, double q2, double q3,
        ...
        if (needsNormalization) {
            ...
            double inv = 1.0 / JdkMath.sqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);
            q0 *= inv;
            q1 *= inv;
            q2 *= inv;
            q3 *= inv;
    ...
    public Rotation(final Vector3D axis, final double angle, final RotationConvention convention)
        ...
        double coeff = JdkMath.sin(halfAngle) / norm;

        q0 = JdkMath.cos (halfAngle);
        q1 = coeff * axis.getX();
        q2 = coeff * axis.getY();
        q3 = coeff * axis.getZ();
    ...
    public Rotation(double[][] m, double threshold)
        ...
        if ((m.length != 3) || (m[0].length != 3) ||
            ...
            throw new NotARotationMatrixException(ROTATION_MATRIX_DIMENSIONS, m.length, m[0].length);
        }
        ...
        if (det < 0.0) {
            throw new NotARotationMatrixException(CLOSEST_ORTHOGONAL_MATRIX_HAS_NEGATIVE_DETERMINANT, det);
        }
    ...
    public Rotation(Vector3D u1, Vector3D u2, Vector3D v1, Vector3D v2)
        ...
        final Vector3D u3 = u1.cross(u2).normalize(); // u1.crossProduct(u2).normalize();
        u2 = u3.cross(u1).normalize();   // u3.crossProduct(u1).normalize();
        u1 = u1.normalize();

        // build an orthonormalized base from v1, v2
        // this fails when vectors are null or collinear, which is forbidden to define a rotation
        final Vector3D v3 = v1.cross(v2).normalize();    // v1.crossProduct(v2).normalize();
        v2 = v3.cross(v1).normalize();   // v3.crossProduct(v1).normalize();
        v1 = v1.normalize();
     ...
     */
    public Rotation(Vector3D u, Vector3D v) throws ArithmeticException {

        double normProduct = u.norm() * v.norm();
        if (normProduct == 0) {
            throw new ArithmeticException(ZERO_NORM_FOR_ROTATION_DEFINING_VECTOR);
        }

        double dot = u.dot(v);   // u.dotProduct(v);
        ...
        if (dot < ((2.0e-15 - 1.0) * normProduct)) {
            ...
            q0 = JdkMath.sqrt(0.5 * (1.0 + dot / normProduct));
            double coeff = 1.0 / (2.0 * q0 * normProduct);
            Vector3D q = v.cross(u); // v.crossProduct(u);
            q1 = coeff * q.getX();
            q2 = coeff * q.getY();
            q3 = coeff * q.getZ();
        }
    ...
    public Vector3D getAxis(final RotationConvention convention) {
        ...
        if (squaredSine == 0) {
            return convention == RotationConvention.VECTOR_OPERATOR ? RotationOrder.PLUS_I : RotationOrder.MINUS_I;
        } else {
            final double sgn = convention == RotationConvention.VECTOR_OPERATOR ? +1 : -1;
            if (q0 < 0) {
                final double inverse = sgn / JdkMath.sqrt(squaredSine);
    ...
    public double getAngle() {
        if ((q0 < -0.1) || (q0 > 0.1)) {
            return 2 * JdkMath.asin(JdkMath.sqrt(q1 * q1 + q2 * q2 + q3 * q3));
        } else if (q0 < 0) {
            return 2 * JdkMath.acos(-q0);
        }
        return 2 * JdkMath.acos(q0);
    }
    ...
    public double[] getAngles(RotationOrder order, RotationConvention convention)
        ...
        if (convention == RotationConvention.VECTOR_OPERATOR) {
            if (order == RotationOrder.XYZ) {
                ...
                // r (RotationOrder.PLUSK) coordinates are :
                //  sin (theta), -cos (theta) sin (phi), cos (theta) cos (phi)
                // (-r) (RotationOrder.PLUSI) coordinates are :
                // cos (psi) cos (theta), -sin (psi) cos (theta), sin (theta)
                // and we can choose to have theta in the interval [-PI/2 ; +PI/2]
                Vector3D v1 = applyTo(RotationOrder.PLUS_K);
                Vector3D v2 = applyInverseTo(RotationOrder.PLUS_I);
                if  ((v2.getZ() < -0.9999999999) || (v2.getZ() > 0.9999999999)) {
                    throw new CardanEulerSingularityException(true);
                }
                return new double[] {
                        JdkMath.atan2(-(v1.getY()), v1.getZ()),
                        JdkMath.asin(v2.getZ()),
                        JdkMath.atan2(-(v2.getY()), v2.getX())
                };

            } else if (order == RotationOrder.XZY) {

                // r (RotationOrder.PLUSJ) coordinates are :
                // -sin (psi), cos (psi) cos (phi), cos (psi) sin (phi)
                // (-r) (RotationOrder.PLUSI) coordinates are :
                // cos (theta) cos (psi), -sin (psi), sin (theta) cos (psi)
                // and we can choose to have psi in the interval [-PI/2 ; +PI/2]
                Vector3D v1 = applyTo(RotationOrder.PLUS_J);
                Vector3D v2 = applyInverseTo(RotationOrder.PLUS_I);
                if ((v2.getY() < -0.9999999999) || (v2.getY() > 0.9999999999)) {
                    throw new CardanEulerSingularityException(true);
                }
                return new double[] {
                        JdkMath.atan2(v1.getZ(), v1.getY()),
                        -JdkMath.asin(v2.getY()),
                        JdkMath.atan2(v2.getZ(), v2.getX())
                };
                ...
                Vector3D v1 = applyTo(RotationOrder.PLUS_K);
                Vector3D v2 = applyInverseTo(RotationOrder.PLUS_J);
                if ((v2.getZ() < -0.9999999999) || (v2.getZ() > 0.9999999999)) {
                    throw new CardanEulerSingularityException(true);
                }
                return new double[] {
                        JdkMath.atan2(v1.getX(), v1.getZ()),
                        -JdkMath.asin(v2.getZ()),
                        JdkMath.atan2(v2.getX(), v2.getY())
                };
                ...
                // and we can choose to have psi in the interval [-PI/2 ; +PI/2]
                Vector3D v1 = applyTo(RotationOrder.PLUS_I);
                Vector3D v2 = applyInverseTo(RotationOrder.PLUS_J);
                if ((v2.getX() < -0.9999999999) || (v2.getX() > 0.9999999999)) {
                    throw new CardanEulerSingularityException(true);
                }
                return new double[] {
                        JdkMath.atan2(-(v1.getZ()), v1.getX()),
                        JdkMath.asin(v2.getX()),
                        JdkMath.atan2(-(v2.getZ()), v2.getY())
                };
                ...
                // and we can choose to have phi in the interval [-PI/2 ; +PI/2]
                Vector3D v1 = applyTo(RotationOrder.PLUS_J);
                Vector3D v2 = applyInverseTo(RotationOrder.PLUS_K);
                if ((v2.getY() < -0.9999999999) || (v2.getY() > 0.9999999999)) {
                    throw new CardanEulerSingularityException(true);
                }
                return new double[] {
                        JdkMath.atan2(-(v1.getX()), v1.getY()),
                        JdkMath.asin(v2.getY()),
                        JdkMath.atan2(-(v2.getX()), v2.getZ())
                };
                ...
                return new double[] {
                        JdkMath.atan2(v1.getY(), v1.getX()),
                        -JdkMath.asin(v2.getX()),
                        JdkMath.atan2(v2.getY(), v2.getZ())
                };
    ...
    private double[][] orthogonalizeMatrix(double[][] m, double threshold)
        ...
        while (++i < 11) {
            ...
            // convergence test
            if (JdkMath.abs(fn1 - fn) <= threshold) {
                return o;
            }
